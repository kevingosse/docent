...
- Base platform: **builds against a locally-installed 2026.3 IDE that bundles Air** (IntelliJ IDEA 263.4953,
...
- **Single artifact, one `src/awb` seam, one Air generation.** Shared Air-free code stays in `src/main`; the
  seam (gated `docent-awb.xml` on plugin id `com.intellij.air`) tracks exactly one Air build — as of 0.8.3
  **263.4953** (bundled in the IDEA 2026.3 nightly). The core review surface is platform-clean and loads across the whole
