...
enum class McpPrereqStatus {
    ...
    /**
     * The endpoint's arming attempt hasn't resolved yet (IDE still starting). Not a problem to warn
     * about — surfaces stay quiet; [DocentMcpPrereqActivity] re-pings them once the attempt settles.
     */
    STARTING,
...
object McpPrereq {
    ...
    fun status(): McpPrereqStatus {
        ...
        return try {
            val endpoint = DocentMcpEndpoint.getInstance()
            if (endpoint.armedOrNull() != null || McpServerGate.isEnabled()) McpPrereqStatus.OK
            else if (!endpoint.settled()) McpPrereqStatus.STARTING
