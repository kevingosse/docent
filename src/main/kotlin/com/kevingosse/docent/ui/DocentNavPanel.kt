...
class DocentNavPanel(private val project: Project) : JPanel(BorderLayout()), Disposable, DocentReviewController.Listener {
    ...
    private fun mcpPrereqRows(): List<JComponent> = when (McpPrereq.status()) {
        // STARTING = the IDE hasn't finished arming the endpoint yet — not a problem, so no scary notice;
        // DocentMcpPrereqActivity re-pings this panel (onConnectionChanged) once the attempt settles.
        McpPrereqStatus.OK, McpPrereqStatus.STARTING -> emptyList()
